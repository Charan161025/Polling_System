...
function view($template, $data = []) {
    ...
    $content = preg_replace_callback('/@foreach\((.+?) as (.+?)\)(.+?)@endforeach/s', function($matches) use ($data) {
        $array = null;
        // Evaluate the array variable from $data (e.g., $polls)
        eval('$array = ' . $matches[1] . ';');
        $varName = trim($matches[2]); // This captures "$p"
        ...
        if (is_array($array)) {
            foreach ($array as $item) {
                ...
                $temp = preg_replace_callback($pattern, function($m) use ($item) {
                    $key = $m[1];
                    return isset($item[$key]) ? htmlspecialchars($item[$key]) : '';
                }, $temp);
...
if ($uri === '/vote' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $db->prepare("INSERT INTO votes (poll_id, option_id, ip_address, voted_at) VALUES (?, ?, ?, datetime('now'))")
       ->execute([$pollId, $optionId, $ip]);
    
    $db->prepare("INSERT INTO vote_history (poll_id, option_id, ip_address, action, timestamp) VALUES (?, ?, ?, 'voted', datetime('now'))")
       ->execute([$pollId, $optionId, $ip]);
...
if ($uri === '/admin') {
    ...
    $history = $db->query("SELECT * FROM vote_history ORDER BY timestamp DESC")->fetchAll(PDO::FETCH_ASSOC);
...
if ($uri === '/release' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $vote = $db->query("SELECT * FROM votes WHERE id=$voteId")->fetch(PDO::FETCH_ASSOC);
    
    if ($vote) {
        $db->exec("DELETE FROM votes WHERE id=$voteId");
        $db->prepare("INSERT INTO vote_history (poll_id, option_id, ip_address, action, timestamp) VALUES (?, ?, ?, 'released', datetime('now'))")
           ->execute([$vote['poll_id'], $vote['option_id'], $vote['ip_address']]);
    }
    echo json_encode(['success' => true]);
    exit;
}
